    ...
    return (
        <div className="space-y-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-700 animate-fadeIn">
                ...
                {result.triggered_rules && result.triggered_rules.length > 0 && (
                    <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                        <p className="text-sm font-semibold text-amber-800 dark:text-amber-300 mb-1">⚡ Business Rules Triggered:</p>
                        <div className="flex flex-wrap gap-1.5">
                            {result.triggered_rules.map((rule, i) => (
                                <span key={i} className="text-xs px-2 py-1 bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 rounded-full">
                                    {rule}
                                </span>
                            ))}
                        </div>
            ...
            {result.shap_explanation && (
                <ShapChart shapData={result.shap_explanation} />
            )}

            {/* Alert Preview for P1/P2 */}
            <AlertPreview result={result} />
        </div>
    );
};
